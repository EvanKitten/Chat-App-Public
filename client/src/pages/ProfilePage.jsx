        ...
        flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
            <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1'>
                ...
                <label htmlFor="avatar" className='flex items-center gap-3 cursor-pointer'>
                    ...
                    <img src={selectImg ? URL.createObjectURL(selectImg) : assets.avatar_icon} alt=""
                    className={`w-12 h-12 ${selectImg && 'rounded-full'}`} />
                    Upload profile image
                </label>
                <input onChange={(e)=> setName(e.target.value)} value={name} type="text" required 
                placeholder='Your name' className='p-2 border border-gray-500 
                ...
                required className="p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 
            ...
            <img className={`max-w-44 aspect-square rounded-full mx-10 max-sm:mt-10 ${selectImg && 'rounded-full'}`} 
